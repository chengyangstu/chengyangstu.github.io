...
按用户 2026-09-08 的要求，试运行阶段只保留最新版 `2026-09-08.json`。三份较早的试验稿已清理，之后的正式周报继续逐期永久保存。旧收藏快照不会随试验稿清理而删除。

- 每期位于 `plasma/data/issues/YYYY-MM-DD.json`（截止日期）。只能新增，重复生成同一期会拒绝覆盖。
...
- 扩充/纠错用 `publish --revision-of 原期ID --briefs 稿件路径`，新文件按原截止日期加 `-r2`、`-r3` 命名，保留原检索窗口并标注扩充版；旧 JSON 不改写。
