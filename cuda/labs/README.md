...
先运行 `python basics.py`。它逐项核对教材中的位置更新、线程索引、CIC 权重、蛙跳半步、Boris 旋转、局部电流连续性、Harris 压力平衡和双分域差分例子。不需 NumPy、CUDA 或服务器；双分域部分仅在 CPU 上模拟 ghost 数据，不能替代 MPI 实测。

第 10 章新增“四粒子完整时间步”：运行时依次打印旧电荷密度、节点电场、粒子插值电场、初始半层速度、推进后速度、新位置、新密度和新电场。对照网页两张账本逐项阅读。为看清每一步，四节点场求解使用标准库直接 Fourier 求和，与 `pic1d.py` 的 FFT 使用相同的零模与 Nyquist 约定；它不是面向大数组的 FFT 实现。只有四个粒子的算例用于说明离散过程，不用于物理增长率测量。

把输出与章节里的手算过程逐项对照，再改一个输入并预测新答案。脚本内断言固定了原始例子的参考结果；更改例子后应先重新推导对应答案，不能删除检查让程序看起来通过。
